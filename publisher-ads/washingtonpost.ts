...
class WPAd extends BATAd {
  ...
  setInventory (inventory: WPAdInventory) {
  ...
  setMatchingInventory() {
    ...
    for (const inventoryKey in window.wpAd.inventory) {
      let hasMatch = (inventoryKey.includes('*'))
        ...
        : (inventoryKey == adKey)
      if (hasMatch) {
        inventoryMatch = window.wpAd.inventory[inventoryKey]
        break
      }
    }
...
runOnPageLoaded(function () {
  function ChooseAdSizesForWP () {
    const adElements = document.querySelectorAll<HTMLElement>('wp-ad')
